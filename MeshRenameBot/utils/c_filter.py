    ...
    if data[1] == "add":
        ...
        if data[2] == "replace":
            ...
            if valg is None:
                ...
                if "|" not in valg:
                    await msg.message.edit_text(
                        fltr_add + "\n\n" + Trans.WRONG_INPUT_FORMAT,
                        reply_markup=markup1,
                    )

                else:
                    valg = valg.split("|", 2)
                    success_add = "\n" + Trans.REPLACE_FILTER_SUCCESS.format(
                        valg[0], valg[1]
                    )
        ...
        if data[2] == "addition":
            if len(data) == 4:
                ...
                inob = userin(client)
                await msg.message.edit_text(
                    Trans.ADDITION_FILTER_INIT_MSG, reply_markup=None
                )
                valg = await inob.get_value(client, msg, del_msg=True)
                valg = valg.strip("|")
                if valg is None:
                    await msg.message.edit_text(
                        fltr_add + "\n\n" + Trans.NO_INPUT_FROM_USER,
                        reply_markup=markup1,
                    )
                    ...
                    if data[3] == "left":
                        success_add = "\n" + Trans.ADDITION_FILTER_SUCCESS_LEFT.format(
                            valg
                        )
                        fsu.add_filer(
                        ...
                        success_add = "\n" + Trans.ADDITION_FILTER_SUCCESS_RIGHT.format(
                            valg
                        )
                    ...
                    [
                        [
                            InlineKeyboardButton(
                                Trans.ADDITION_LEFT, "fltr addf addition left"
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                Trans.ADDITION_RIGHT, "fltr addf addition right"
                            )
                        ],
                ...
                await msg.message.edit_text(
                    Trans.ADDITION_POSITION_PROMPT, reply_markup=addition_markup
                )
